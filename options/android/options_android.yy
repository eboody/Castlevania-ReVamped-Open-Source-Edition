{
  "$GMAndroidOptions":"",
  "%Name":"Android",
  "name":"Android",
  "option_android_application_tag_inject":"",
  "option_android_arch_arm64":true,
  "option_android_arch_armv7":false,
  "option_android_arch_x86_64":false,
  "option_android_attribute_allow_backup":false,
  "option_android_build_tools":"35.0.0",
  "option_android_compile_sdk":"35",
  "option_android_device_support":0,
  "option_android_display_name":"Castlevania ReVamped",
  "option_android_facebook_app_display_name":"",
  "option_android_facebook_id":"",
  "option_android_gamepad_support":true,
  "option_android_google_apk_expansion":false,
  "option_android_google_cloud_saving":false,
  "option_android_google_dynamic_asset_delivery":false,
  "option_android_google_licensing_public_key":"",
  "option_android_google_services_app_id":"",
  "option_android_icon_adaptivebg_hdpi":"${options_dir}/../icon.png",
  "option_android_icon_adaptivebg_ldpi":"${options_dir}/../icon.png",
  "option_android_icon_adaptivebg_mdpi":"${options_dir}/../icon.png",
  "option_android_icon_adaptivebg_xhdpi":"${options_dir}/../icon.png",
  "option_android_icon_adaptivebg_xxhdpi":"${options_dir}/../icon.png",
  "option_android_icon_adaptivebg_xxxhdpi":"${options_dir}/../icon.png",
  "option_android_icon_adaptive_generate":false,
  "option_android_icon_adaptive_hdpi":"${options_dir}/../icon.png",
  "option_android_icon_adaptive_ldpi":"${options_dir}/../icon.png",
  "option_android_icon_adaptive_mdpi":"${options_dir}/../icon.png",
  "option_android_icon_adaptive_xhdpi":"${options_dir}/../icon.png",
  "option_android_icon_adaptive_xxhdpi":"${options_dir}/../icon.png",
  "option_android_icon_adaptive_xxxhdpi":"${options_dir}/../icon.png",
  "option_android_icon_hdpi":"${options_dir}/../icon.png",
  "option_android_icon_ldpi":"${options_dir}/../icon.png",
  "option_android_icon_mdpi":"${options_dir}/../icon.png",
  "option_android_icon_xhdpi":"${options_dir}/../icon.png",
  "option_android_icon_xxhdpi":"${options_dir}/../icon.png",
  "option_android_icon_xxxhdpi":"${options_dir}/../icon.png",
  "option_android_install_location":0,
  "option_android_interpolate_pixels":false,
  "option_android_launchscreen_fill":0,
  "option_android_lint":false,
  "option_android_logcat":"yoyo:V DEBUG:V AndroidRuntime:V",
  "option_android_minimum_sdk":"23",
  "option_android_orient_landscape":true,
  "option_android_orient_landscape_flipped":true,
  "option_android_orient_portrait":false,
  "option_android_orient_portrait_flipped":false,
  "option_android_package_company":"lv4games",
  "option_android_package_domain":"com",
  "option_android_package_product":"castlevaniarevamped",
  "option_android_permission_bluetooth":false,
  "option_android_permission_internet":false,
  "option_android_permission_network_state":false,
  "option_android_permission_read_phone_state":false,
  "option_android_permission_record_audio":false,
  "option_android_permission_write_external_storage":false,
  "option_android_proguard_minifying":false,
  "option_android_proguard_shrinking":false,
  "option_android_scale":0,
  "option_android_screen_depth":0,
  "option_android_sleep_margin":4,
  "option_android_splashscreen_background_colour":4278190080,
  "option_android_splash_screens_landscape":"${options_dir}/../splash.png",
  "option_android_splash_screens_portrait":"${options_dir}/../splash.png",
  "option_android_splash_time":0,
  "option_android_support_lib":"",
  "option_android_sync_amazon":false,
  "option_android_target_sdk":"35",
  "option_android_texture_page":"2048x2048",
  "option_android_tools_from_version":false,
  "option_android_tv_banner":"${base_options_dir}/android/tv_banner.png",
  "option_android_tv_isgame":true,
  "option_android_tv_supports_leanback":true,
  "option_android_use_facebook":false,
  "option_android_version":"1.0.0.0",
  "resourceType":"GMAndroidOptions",
  "resourceVersion":"2.0",
}